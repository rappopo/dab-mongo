  ...
  createCollection (coll, params) {
    ...
    return new Promise((resolve, reject) => {
      super.createCollection(coll)
        ...
        .catch(err => {
          if (err.message !== 'Collection not found') return reject(err)
          this.getClientDb()
            .then(db => {
    ...
    return new Promise((resolve, reject) => {
      ...
      super.renameCollection(oldName, newName)
        ...
        .then(result => {
          ...
          if (!rebuild) return resolve({ success: true })
      ...
      this.getClientDb()
        .then(db => {
          db.collection(name, { strict: true }, (err, result) => {
            if (err) {
              if (err.message.indexOf('does not exist') > -1) err = new Error('Collection not found')
              return reject(err)
            }
            resolve(result)
          })
  ...
  create (body, params) {
    ...
    return new Promise((resolve, reject) => {
      this.getCollection(params.collection)
        ...
        .then(result => {
          this._findOne(result.insertedId, params, (e, resp) => {
            if (resp.success) {
              resolve({
                success: true,
        ...
        .catch(err => {
          if (err.message.indexOf('duplicate key error collection') > -1) err = new Error('Document already exists')
          reject(err)
        })
    })
  }
          ...
          coll.find({
            ...
            coll.insertMany(newBody, (err, result) => {
              ...
              this._.each(body, (r, i) => {
                let stat = { success: info.indexOf(r._id) === -1 }
                stat._id = r._id
